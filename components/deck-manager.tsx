...
export function DeckManager({ onAddDeck, onRefreshDecks }: DeckManagerProps) {
  ...
  const handleJsonSubmit = () => {
    ...
    try {
      ...
      const filename = data.name ? `${data.name.toLowerCase().replace(/\s+/g, "-")}.json` : "custom-deck.json"
  ...
  const downloadSampleDeck = () => {
    const sampleDeck = {
      ...
      cards: [
        ...
        },
        {
          ...
          back: "Pacific Ocean",
          example: "The Pacific Ocean covers about 46% of the world's water surface.",
    ...
    const blob = new Blob([JSON.stringify(sampleDeck, null, 2)], { type: "application/json" })
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Upload a JSON file containing flashcard data</p>
        </div>

        <div className="space-y-2">
          <Label>Paste JSON Data</Label>
          <div className="space-y-2">
            <Button onClick={() => setShowJsonInput(!showJsonInput)} variant="outline" size="sm" className="w-full">
        ...
        <div className="space-y-2">
          ...
          <div className="text-xs text-muted-foreground space-y-1">
            ...
            <ul className="list-disc list-inside ml-2 space-y-1">
              ...
              <li>
                Object with metadata: <code>{`{"name": "...", "cards": [...]}`}</code>
              </li>
            </ul>
            <p className="mt-2">
              Each card must have: <code>id</code>, <code>front</code>, <code>back</code>
            </p>
            <p>
              Optional fields: <code>example</code>, <code>mnemonic</code>, <code>category</code>, <code>subject</code>,{" "}
              <code>course</code>, <code>module</code>, <code>difficulty</code>, <code>tags</code>
            </p>
          </div>
        </div>
      </CardContent>
